...
async function readCardWithTimestamps(filePath) {
  // The path can disappear after a board directory has been enumerated. Read
  // the content first so a later stat race does not discard a valid card read.
...
async function readCardIfPresent(filePath) {
  try {
    return await readCardWithTimestamps(filePath);
  } catch (error) {
