    ...
    try {
      ...
      for (const listName of Array.isArray(listNames) ? listNames : []) {
        if (v2Profile) {
          ...
          if (!stageSemantics.mapped || stageSemantics.ambiguous || stageSemantics.terminal) {
            continue;
          }
        } else if (isCompletedListByWorkflow(listName, workflowSettings)) {
          continue;
        }
        ...
        for (const cardName of Array.isArray(cardNames) ? cardNames : []) {
          ...
          const cardTitle = getCardDisplayTitle(frontmatter.title, cardName);
