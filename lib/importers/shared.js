...
const { listOrderedEntries, writeOrderManifest } = require('../orderManifest');
...
async function createImportContext(boardRoot, importer, sourcePaths = []) {
  ...
  for (const entry of existingEntries) {
    ...
    const displayName = match && match[1] ? match[1] : String(entry.name).replace(/^\d+-/, '');
    if (displayName) existingListNames.add(displayName.toLowerCase());
  }
...
async function createList(context, rawName) {
  ...
  const directoryName = sanitizeListName(displayName);
  ...
  if (!orderedLists.includes(directoryName)) {
    await writeOrderManifest(context.boardRoot, [...orderedLists, directoryName]);
  }
...
async function createCard(context, listEntry, card = {}) {
  ...
  const fileName = `${sanitizeCardSlug(title).slice(0, 40)}-${randomSuffix()}.md`;
  const filePath = path.join(targetList.path, fileName);
  const normalizedFrontmatter = prepareNewCardFrontmatter({
    title,
    start: normalizeIsoDateFromValue(card.start),
