...
} = require('./cardTimestamps');
const {
  archiveCard: archiveCardOnDisk,
  archiveList: archiveListOnDisk,
  listArchiveEntries: listArchiveEntriesOnDisk,
  readArchiveEntry: readArchiveEntryOnDisk,
  restoreArchivedCard: restoreArchivedCardOnDisk,
  restoreArchivedList: restoreArchivedListOnDisk,
} = require('./archive');
...
const { getListDisplayName, prepareNewCardFrontmatter, recordCardListMove: applyCardListMove } = require('./cardLifecycle');
const {
  appendNote,
  insertAfterHeading,
  replaceSection,
...
const MCP_BOARD_VIEW_OPTIONS = Object.freeze([
  ...
  Object.freeze({
    id: 'table',
    label: 'Table',
    description: 'Board-scoped table view for scanning cards and moving cards between lists.',
  }),
]);

function toPublicToolName(name) {
  const normalized = String(name || '').trim();
  if (normalized.startsWith('signboard.')) {
    return `signboard_${normalized.slice('signboard.'.length)}`;
  }
  return normalized;
}
...
function toInternalToolName(name) {
  const normalized = String(name || '').trim();
  if (normalized.startsWith('signboard_')) {
    return `signboard.${normalized.slice('signboard_'.length)}`;
  }
  return normalized;
}
...
const INTERNAL_TOOL_DEFINITIONS = [
  ...
  {
    name: 'signboard.list_boards',
    description: 'List known usable Signboard board roots from desktop-open, desktop-trusted, and configured MCP roots.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      properties: {
        includeAllowedRootScan: {
          type: 'boolean',
          description: 'Scan configured allowed roots for board-looking folders. Defaults to true.',
        },
        maxDepth: {
          type: 'integer',
          description: `Allowed-root scan depth, 0-${MAX_BOARD_SCAN_DEPTH}. Defaults to ${DEFAULT_BOARD_SCAN_DEPTH}.`,
        },
        limit: {
          type: 'integer',
          description: `Maximum boards to return, 1-${MAX_BOARD_SCAN_LIMIT}. Defaults to ${DEFAULT_BOARD_SCAN_LIMIT}.`,
        },
      },
    },
  },
  ...
  },
  {
    ...
    description: 'Create a new board directory with default workflow lists and an optional starter card.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        cardFile: { type: 'string', description: 'Markdown file name, e.g. task-abc12.md' },
      },
    },
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        boardRoot: { type: 'string' },
        listName: { type: 'string' },
        title: { type: 'string', description: 'Optional explicit title override. Omit to use the filename-derived display title.' },
        body: { type: 'string' },
        start: { type: ['string', 'null'], description: 'Optional start/scheduled date. Use YYYY-MM-DD when possible.' },
        ...
        kind: { type: 'string', description: 'Optional V2 kind: task, discovery, epic, or incident.' },
        priorityClass: { type: 'string', description: 'Optional V2 priority class: P0-P3.' },
        effortPoints: { type: ['number', 'null'], description: 'Optional V2 effort estimate.' },
        dryRun: { type: 'boolean', description: 'Preview the created card without writing a new file.' },
      },
    },
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        replaceSection: { type: 'string', description: 'Markdown heading text or heading line whose section body should be replaced with body.' },
        insertAfterHeading: { type: 'string', description: 'Markdown heading text or heading line after which insertText should be inserted.' },
        insertText: { type: 'string' },
        addNote: { type: 'string', description: 'Append a list-style note under the Notes section, creating it if needed.' },
        noteTimestamp: { type: 'boolean', description: 'Prefix addNote with the same timestamp format used by the desktop editor.' },
        noteSection: { type: 'string', description: 'Heading section for addNote. Defaults to Notes.' },
        dryRun: { type: 'boolean', description: 'Preview the update without writing the card file.' },
      },
    },
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        title: { type: 'string', description: 'Exact title for the duplicated card. Overrides titlePrefix when provided.' },
        body: { type: 'string', description: 'Optional replacement body for the duplicated card.' },
        removeLabelIds: {
          type: 'array',
          items: { type: 'string' },
        ...
        addLabelIds: {
          type: 'array',
          items: { type: 'string' },
          description: 'Optional label ids to add to the duplicate.',
        },
        clearLabels: { type: 'boolean', description: 'Clear all copied labels before adding labels.' },
        dryRun: { type: 'boolean', description: 'Preview the duplicate without writing a new card file.' },
      },
    },
  },
  {
  ...
  {
    name: 'signboard.archive_list',
    description: 'Move a list into XXX-Archive (creates archive list when missing).',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      required: ['boardRoot', 'listName'],
  ...
  {
    name: 'signboard.list_archive_entries',
    description: 'List archived cards and archived lists for a board.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      required: ['boardRoot'],
  ...
  {
    name: 'signboard.read_archive_entry',
    description: 'Read one archived card or archived list entry by absolute archive path.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      required: ['boardRoot', 'entryPath'],
      properties: {
        boardRoot: { type: 'string' },
        entryPath: { type: 'string', description: 'Absolute archived card or archived list path inside XXX-Archive.' },
      },
    },
  },
  {
    name: 'signboard.restore_archived_card',
    description: 'Restore an archived card into a destination list at the top of that list.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      required: ['boardRoot', 'archivedCardPath', 'targetListName'],
      properties: {
        boardRoot: { type: 'string' },
        archivedCardPath: { type: 'string', description: 'Absolute archived card path inside XXX-Archive.' },
        targetListName: { type: 'string', description: 'Destination list directory name inside the board root.' },
      },
    },
  },
  {
    name: 'signboard.restore_archived_list',
    description: 'Restore an archived list back into the board root, optionally with a different directory name.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      required: ['boardRoot', 'archivedListPath'],
      properties: {
        boardRoot: { type: 'string' },
        archivedListPath: { type: 'string', description: 'Absolute archived list directory path inside XXX-Archive.' },
        restoredDirectoryName: { type: 'string', description: 'Optional destination directory name when restoring the list.' },
      },
    },
  },
  {
  ...
  },
  {
    ...
    description: 'Read board label, theme, workflow, V2 profile, and External Published Calendar inclusion settings from the board root .board.json manifest.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      required: ['boardRoot'],
  ...
  },
  {
    ...
    description: 'Update board settings (labels, themeOverrides, workflow, V2 profile, and/or External Published Calendar inclusion).',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      required: ['boardRoot'],
      properties: {
        ...
        v2: { type: 'object' },
        externalPublishedCalendar: { type: 'object' },
      },
    },
  },
  {
  ...
  },
  {
    ...
    description: 'Import a Tasks.md project directory into an existing Signboard board.',
    inputSchema: {
      type: 'object',
      additionalProperties: false,
      required: ['boardRoot', 'sourcePath'],
      properties: {
        boardRoot: { type: 'string', description: 'Absolute board root path on disk.' },
        sourcePath: { type: 'string', description: 'Absolute path to a Tasks.md project directory.' },
      },
    },
  },
];

const TOOL_DEFINITIONS = INTERNAL_TOOL_DEFINITIONS.map((toolDefinition) => ({
  ...toolDefinition,
  name: toPublicToolName(toolDefinition.name),
}));
...
function normalizeTaskStartDateValue(dateValue) {
  return normalizeTaskDueDateValue(dateValue);
}
...
function summarizeTaskList(bodyValue) {
  ...
  for (const lineValue of lines) {
    ...
    let content = String(match[3] || '').replace(/^\s+/, '');
    while (content) {
      const dateMatch = content.match(TASK_DATE_MARKER_PATTERN);
      if (!dateMatch) {
        break;
      }

      const markerName = String(dateMatch[1] || '').trim().toLowerCase();
      const dateValue = markerName === 'due'
        ? normalizeTaskDueDateValue(dateMatch[2])
        : normalizeTaskStartDateValue(dateMatch[2]);
      if (dateValue) {
        if (markerName === 'due') {
          dueSet.add(dateValue);
        } else {
          startSet.add(dateValue);
        }
      }
...
function normalizeAllowedRootList(values) {
  const seen = new Set();
  const roots = [];
...
function parseAllowedRoots() {
  ...
  return normalizeAllowedRootList(raw.split(path.delimiter));
}

function mergeAllowedRoots(...rootGroups) {
  return normalizeAllowedRootList(rootGroups.flatMap((roots) => (Array.isArray(roots) ? roots : [])));
}
...
function assertInsideAllowedRoots(config, candidatePath, fieldName) {
  if (!Array.isArray(config.allowedRoots) || config.allowedRoots.length === 0) {
    throw new Error(`${fieldName} requires SIGNBOARD_MCP_ALLOWED_ROOTS to be configured.`);
  }
...
async function resolveExistingPath(config, pathInput, fieldName) {
  const resolvedPath = resolveAbsolutePathInput(pathInput, fieldName);
  await ensureExistingFileOrDirectory(resolvedPath, `${fieldName} does not exist: ${resolvedPath}`);
  assertInsideAllowedRoots(config, resolvedPath, fieldName);
  return resolvedPath;
}
...
async function resolveBoardPathsByName(config, boardName, options) {
  if (!Array.isArray(config.allowedRoots) || config.allowedRoots.length === 0) {
    throw new Error('Board name resolution requires trusted board roots or SIGNBOARD_MCP_ALLOWED_ROOTS to be configured.');
  }
  ...
  for (const rootPath of config.allowedRoots) {
    const rootName = path.basename(rootPath);
    if (boardNameMatches(rootName, boardName, options)) {
      matches.push(rootPath);
      seen.add(rootPath);
...
function buildServerConfig(options = {}) {
  const trustedBoardRoots = normalizeAllowedRootList(options.trustedBoardRoots);
  const desktopOpenBoardsState = normalizeOpenBoardsState(options.desktopOpenBoardsState);
  return {
    appVersion: String(options.appVersion || '0.0.0'),
    readOnly: parseBoolean(process.env.SIGNBOARD_MCP_READ_ONLY, DEFAULT_READ_ONLY),
    allowedRoots: mergeAllowedRoots(parseAllowedRoots(), trustedBoardRoots),
    trustedBoardRoots,
...
async function handleToolCall(config, name, args = {}) {
  const toolName = toInternalToolName(name);
  ...
  switch (toolName) {
    case 'signboard.get_config': {
      return {
        ...
        trustedBoardRoots: config.trustedBoardRoots,
        openBoardRoots: config.desktopOpenBoardsState.openBoardRoots,
        activeBoardRoot: config.desktopOpenBoardsState.activeBoardRoot,
    ...
    case 'signboard.list_boards': {
      return discoverBoards({
        allowedRoots: config.allowedRoots,
        trustedBoardRoots: config.trustedBoardRoots,
        openBoardRoots: config.desktopOpenBoardsState.openBoardRoots,
        activeBoardRoot: config.desktopOpenBoardsState.activeBoardRoot,
        includeAllowedRootScan: args.includeAllowedRootScan !== false,
    ...
    case 'signboard.create_board': {
      assertWriteAllowed(config, toPublicToolName(toolName));
      ...
      return createBoardOnDisk(boardRoot, { seedWelcomeCard: shouldSeedWelcomeCard });
    }
    ...
    case 'signboard.list_lists': {
      ...
      const listNames = await listOrderedEntries(
        boardRoot,
    ...
    case 'signboard.list_cards': {
      ...
      const cardFiles = await listOrderedEntries(
        listPath,
    ...
    case 'signboard.create_card': {
      ...
      const slug = slugifyTitle(title || 'card').slice(0, 24);
      const cardFile = `${slug}-${randomSuffix()}.md`;
      ...
      if (Object.prototype.hasOwnProperty.call(args, 'start')) {
        if (args.start == null || String(args.start).trim() === '') {
          frontmatter.start = undefined;
        } else {
          frontmatter.start = String(args.start).trim();
        }
      }
      ...
      const hasV2Overrides = args.kind != null || args.priorityClass != null || args.effortPoints != null;
      if (v2Profile || hasV2Overrides) {
        const defaults = v2Profile && v2Profile.cardDefaults ? v2Profile.cardDefaults : {};
        const metadata = {
          contract_version: 1,
          kind: String(args.kind || defaults.kind || 'task'),
          priority_class: String(args.priorityClass || defaults.priorityClass || 'P2'),
        };
        if (args.effortPoints != null && Number.isFinite(Number(args.effortPoints))) metadata.estimate = { effort_points: Number(args.effortPoints) };
        frontmatter.signboard_v2 = metadata;
      }

      const nextFrontmatter = cardFrontmatter.normalizeFrontmatter(prepareNewCardFrontmatter(frontmatter));
      const nextBody = normalizeCardBodyInput(args.body);

      if (isDryRun) {
        const taskListMetadata = summarizeTaskList(nextBody);
        return {
          ok: true,
          dryRun: true,
          boardRoot,
          ...
          cardFile,
          card: {
            displayTitle: getCardDisplayTitle(nextFrontmatter.title, cardFile),
            frontmatter: nextFrontmatter,
            body: nextBody,
            timestamps: resolveCardTimestamps(nextFrontmatter),
          },
          ...taskListMetadata,
        };
      }
      ...
      if (!orderedCards.includes(cardFile)) {
        await writeOrderManifest(listPath, [...orderedCards, cardFile]);
      }
    ...
    case 'signboard.update_card': {
      ...
      const current = await readCardWithTimestamps(cardPath);
      ...
      if (Object.prototype.hasOwnProperty.call(args, 'start')) {
        if (args.start == null || String(args.start).trim() === '') {
          delete nextFrontmatter.start;
        } else {
          nextFrontmatter.start = String(args.start).trim();
        }
      }
      ...
      if (Object.prototype.hasOwnProperty.call(args, 'labels')) {
        ...
        const labelSet = new Set(
          args.clearLabels === true
            ? []
            : (Array.isArray(nextFrontmatter.labels) ? nextFrontmatter.labels.map((value) => String(value)) : [])
        );

        if (Array.isArray(args.addLabelIds)) {
          for (const labelId of args.addLabelIds) {
            labelSet.add(String(labelId));
          }
        }

        if (Array.isArray(args.removeLabelIds)) {
          for (const labelId of args.removeLabelIds) {
            labelSet.delete(String(labelId));
          }
        }

        nextFrontmatter.labels = [...labelSet];
      }
      ...
      if (Object.prototype.hasOwnProperty.call(args, 'replaceSection')) {
        if (!Object.prototype.hasOwnProperty.call(args, 'body')) {
          throw new Error('replaceSection requires body to contain the replacement section text.');
        }

        nextBody = replaceSection(current.body, args.replaceSection, normalizeCardBodyInput(args.body));
      }

      if (Object.prototype.hasOwnProperty.call(args, 'insertAfterHeading')) {
        if (!Object.prototype.hasOwnProperty.call(args, 'insertText')) {
          throw new Error('insertAfterHeading requires insertText.');
        }

        nextBody = insertAfterHeading(nextBody, args.insertAfterHeading, normalizeCardBodyInput(args.insertText));
      }

      if (Object.prototype.hasOwnProperty.call(args, 'addNote')) {
        nextBody = appendNote(nextBody, args.addNote, {
          section: args.noteSection || 'Notes',
          timestamp: Boolean(args.noteTimestamp),
        });
      }
        ...
        return {
          ...
          card: {
            ...
            timestamps: {
              ...
              updatedAt: current.timestamps ? current.timestamps.updatedAt : '',
              updatedAtSource: current.timestamps ? current.timestamps.updatedAtSource : '',
            },
          },
          ...taskListMetadata,
        };
    ...
    case 'signboard.duplicate_card': {
      ...
      const sourceTitle = getCardDisplayTitle(sourceCard?.frontmatter?.title, source.cardFile);
      const titlePrefix = Object.prototype.hasOwnProperty.call(args, 'titlePrefix')
        ? String(args.titlePrefix || '')
        : 'Copy of ';
      const nextTitle = Object.prototype.hasOwnProperty.call(args, 'title')
        ? String(args.title || '').trim()
        : (`${titlePrefix}${sourceTitle}`.trim() || sourceTitle);
      const removeLabelIds = Array.isArray(args.removeLabelIds) ? new Set(args.removeLabelIds.map((value) => String(value))) : new Set();
      const addLabelIds = Array.isArray(args.addLabelIds) ? args.addLabelIds.map((value) => String(value)) : [];
      const labelsSet = new Set(
        args.clearLabels === true
          ? []
          : (Array.isArray(sourceCard.frontmatter?.labels) ? sourceCard.frontmatter.labels.map((value) => String(value)) : [])
      );

      for (const labelId of removeLabelIds) {
        labelsSet.delete(labelId);
      }
      ...
      const duplicatedBody = Object.prototype.hasOwnProperty.call(args, 'body')
        ? normalizeCardBodyInput(args.body)
        : sourceCard.body;
      const duplicatedCardFile = `${slugifyTitle(nextTitle || 'card').slice(0, 24)}-${randomSuffix()}.md`;
      const duplicatedCardPath = path.join(targetListPath, duplicatedCardFile);
      const duplicatedFrontmatter = prepareNewCardFrontmatter({
      ...
      if (isDryRun) {
        const taskListMetadata = summarizeTaskList(duplicatedBody);
        return {
          ok: true,
          dryRun: true,
          ...
          card: {
            displayTitle: getCardDisplayTitle(duplicatedFrontmatter.title, duplicatedCardFile),
            frontmatter: duplicatedFrontmatter,
            body: duplicatedBody,
            timestamps: resolveCardTimestamps(duplicatedFrontmatter),
          },
          ...taskListMetadata,
        };
      }
      ...
      if (!orderedCards.includes(duplicatedCardFile)) {
        await writeOrderManifest(targetListPath, [...orderedCards, duplicatedCardFile]);
      }

      const duplicatedCard = await readCardWithTimestamps(duplicatedCardPath);
    ...
    case 'signboard.archive_card': {
      ...
      const archived = await archiveCardOnDisk(source.boardRoot, source.cardPath);
      ...
      return {
        ...
        toListName: ARCHIVE_DIRECTORY_NAME,
        cardFile: source.cardFile,
        archivedCardFile: archived.archivedCardFile,
        archivedCardPath: archived.archivedCardPath,
      };
    }

    case 'signboard.archive_list': {
      assertWriteAllowed(config, toPublicToolName(toolName));

      const { boardRoot, listName, listPath } = await resolveListPath(config, args.boardRoot, args.listName);
      await ensureExistingDirectory(listPath, `List does not exist: ${listName}`);

      if (listName === ARCHIVE_DIRECTORY_NAME) {
        return {
          ok: true,
          boardRoot,
          listName,
          archivedDirectoryName: ARCHIVE_DIRECTORY_NAME,
          archivedListPath: listPath,
          alreadyArchived: true,
        };
      }

      const archived = await archiveListOnDisk(boardRoot, listPath);
      return {
        ok: true,
        boardRoot,
        listName,
        archivedDirectoryName: archived.archivedDirectoryName,
        archivedListPath: archived.archivedListPath,
      };
    }

    case 'signboard.list_archive_entries': {
      const boardRoot = await resolveBoardRoot(config, args.boardRoot);
      return listArchiveEntriesOnDisk(boardRoot);
    }

    case 'signboard.read_archive_entry': {
      const boardRoot = await resolveBoardRoot(config, args.boardRoot);
      const entryPath = await resolveExistingPath(config, args.entryPath, 'entryPath');
      return readArchiveEntryOnDisk(boardRoot, entryPath);
    }

    case 'signboard.restore_archived_card': {
      assertWriteAllowed(config, toPublicToolName(toolName));

      const boardRoot = await resolveBoardRoot(config, args.boardRoot);
      const archivedCardPath = await resolveExistingPath(config, args.archivedCardPath, 'archivedCardPath');
      const { listName: targetListName, listPath: targetListPath } = await resolveListPath(config, boardRoot, args.targetListName);
      await ensureExistingDirectory(targetListPath, `List does not exist: ${targetListName}`);

      const restored = await restoreArchivedCardOnDisk(boardRoot, archivedCardPath, targetListPath);
      return {
        ok: true,
        boardRoot,
        archivedCardPath: restored.archivedCardPath,
        restoredCardPath: restored.restoredCardPath,
        restoredCardFile: restored.restoredCardFile,
        targetListName,
        targetListDisplayName: restored.targetListDisplayName,
        removedEmptyArchiveList: restored.removedEmptyArchiveList,
      };
    }

    case 'signboard.restore_archived_list': {
      assertWriteAllowed(config, toPublicToolName(toolName));

      const boardRoot = await resolveBoardRoot(config, args.boardRoot);
      const archivedListPath = await resolveExistingPath(config, args.archivedListPath, 'archivedListPath');
      const restoredDirectoryName = Object.prototype.hasOwnProperty.call(args, 'restoredDirectoryName')
        ? String(args.restoredDirectoryName || '')
        : '';
      const restored = await restoreArchivedListOnDisk(boardRoot, archivedListPath, restoredDirectoryName);

      return {
        ok: true,
        boardRoot,
        archivedListPath: restored.archivedListPath,
        restoredListPath: restored.restoredListPath,
        restoredDirectoryName: restored.restoredDirectoryName,
        restoredListDisplayName: restored.restoredListDisplayName,
      };
    }
    ...
    case 'signboard.move_card': {
      ...
      if (
        ...
        fromListName !== ARCHIVE_DIRECTORY_NAME &&
        toListName !== ARCHIVE_DIRECTORY_NAME
      ) {
        const finalDestinationPath = path.join(toListPath, finalDestinationFile);
        const movedCard = await cardFrontmatter.readCard(finalDestinationPath);
        await cardFrontmatter.writeCard(finalDestinationPath, {
          frontmatter: applyCardListMove(movedCard.frontmatter, {
            fromListDirectoryName: fromListName,
            fromListDisplayName: getListDisplayName(fromListName),
            toListDirectoryName: toListName,
            toListDisplayName: getListDisplayName(toListName),
          }),
          body: movedCard.body,
        });
      }
    ...
    case 'signboard.create_list': {
      ...
      if (!orderedLists.includes(listName)) {
        await writeOrderManifest(boardRoot, [...orderedLists, listName]);
      }
    ...
    case 'signboard.update_board_settings': {
      ...
      if (Object.prototype.hasOwnProperty.call(args, 'workflow')) {
        nextSettings.workflow = args.workflow;
      }

      if (Object.prototype.hasOwnProperty.call(args, 'v2')) {
        nextSettings.v2 = args.v2;
      }

      if (Object.prototype.hasOwnProperty.call(args, 'externalPublishedCalendar')) {
        nextSettings.externalPublishedCalendar = args.externalPublishedCalendar;
      }
    ...
    case 'signboard.import_tasksmd': {
      assertWriteAllowed(config, toPublicToolName(toolName));

      const boardRoot = await resolveBoardRoot(config, args.boardRoot);
      const sourcePath = await resolveExistingImportSourcePath(config, args.sourcePath, 'sourcePath');
