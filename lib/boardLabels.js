...
function cloneDefaultV2Profile() {
  return {
    ...
    retainPlanner: DEFAULT_V2_PROFILE.retainPlanner,
  };
}
...
function normalizeV2Profile(rawProfile = {}) {
  ...
  normalized.retainPlanner = typeof source.retainPlanner === 'boolean'
    ? source.retainPlanner
    : defaults.retainPlanner;
...
function v2ProfileHasValues(profile) {
  ...
  if (Object.keys(normalized.cardDefaults).some((key) => !['kind', 'workType', 'priorityClass'].includes(key))) {
    return true;
  }

  if (normalized.enabled || normalized.profileId !== defaults.profileId || normalized.title !== defaults.title || normalized.description !== defaults.description || normalized.validationPolicy !== defaults.validationPolicy || normalized.retainPlanner !== defaults.retainPlanner) {
    return true;
  }
