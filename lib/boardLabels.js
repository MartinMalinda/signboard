...
const { normalizeExecutionCeiling } = require('../shared/v2ExecutionPolicy');
...
const DEFAULT_V2_EXECUTION_CEILING = 'human_only';
const DEFAULT_V2_BACKGROUND_SELECTION = false;
...
const DEFAULT_V2_PROFILE = Object.freeze({
  ...
  cardDefaults: Object.freeze({
    ...
    workType: 'product',
    priorityClass: 'P2',
    executionCeiling: DEFAULT_V2_EXECUTION_CEILING,
    backgroundSelection: DEFAULT_V2_BACKGROUND_SELECTION,
  }),
...
function normalizeV2CardDefaults(rawDefaults) {
  ...
  normalized.workType = normalizeV2String(rawDefaults && rawDefaults.workType, DEFAULT_V2_PROFILE.cardDefaults.workType) || DEFAULT_V2_PROFILE.cardDefaults.workType;
  ...
  normalized.executionCeiling = normalizeExecutionCeiling(rawDefaults && rawDefaults.executionCeiling);
  normalized.backgroundSelection = typeof rawDefaults?.backgroundSelection === 'boolean'
    ? rawDefaults.backgroundSelection
    : DEFAULT_V2_PROFILE.cardDefaults.backgroundSelection;
  return normalized;
}
...
function v2ProfileHasValues(profile) {
  ...
  if (Object.keys(normalized.cardDefaults).some((key) => !['kind', 'workType', 'priorityClass', 'executionCeiling', 'backgroundSelection'].includes(key))) {
    return true;
  }
