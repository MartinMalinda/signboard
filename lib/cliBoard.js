...
async function createCard(boardRoot, options = {}) {
  ...
  const hasV2Overrides = options.kind != null || options.workType != null || options.priorityClass != null || options.effortPoints != null;
  if (v2Profile || hasV2Overrides) {
    ...
    initialFrontmatter.signboard_v2 = {
      ...
      work_type: String(options.workType || defaults.workType || 'product'),
...
async function duplicateCard(boardRoot, options = {}) {
  ...
  const sourceTitle = String(sourceCard.frontmatter?.title || sourceRecord.title || 'Untitled').trim() || 'Untitled';
  ...
  const body = Object.prototype.hasOwnProperty.call(options, 'body')
    ? String(options.body || '')
    : sourceCard.body;
  const prefix = await nextCardPrefix(targetList.path);
  const fileName = `${prefix}-${sanitizeCardSlug(title).slice(0, 25)}-${randomSuffix()}.md`;
...
async function editCard(boardRoot, options = {}) {
  ...
  if (options.dryRun === true) {
    ...
    return {
      ...
      title: String(nextFrontmatter.title || '').trim() || cardRecord.title,
