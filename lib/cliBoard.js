...
async function listLists(boardRoot, options = {}) {
  ...
  const snapshot = await readBoardSnapshot(resolvedBoardRoot, {
    includeArchive,
  ...
  return snapshot.lists.map((list) => ({
    directoryName: list.listName,
    displayName: getListDisplayName(list.listName),
    path: list.listPath,
    isArchive: list.listName === ARCHIVE_DIRECTORY_NAME,
    cardCount: withCardCounts ? list.cards.length : null,
  }));
}
...
async function loadCardRecord(boardRoot, listEntry, fileName) {
  ...
  return {
    ...
    labels,
    labelNames: await labelNamesForIds(boardRoot, labels),
...
function cardRecordFromSnapshot(boardRoot, listEntry, snapshotCard) {
  const frontmatter = snapshotCard && snapshotCard.frontmatter && typeof snapshotCard.frontmatter === 'object'
    ? snapshotCard.frontmatter
    : {};
  const timestamps = snapshotCard && snapshotCard.timestamps && typeof snapshotCard.timestamps === 'object'
    ? snapshotCard.timestamps
    : {};
  const fileName = String(snapshotCard && snapshotCard.cardName || '');
  const body = String(snapshotCard && snapshotCard.body || '');
  ...
  return {
    ...
    listPath: listEntry.path,
    fileName,
    filePath: snapshotCard.cardPath,
    cardId: getCardId(fileName),
    title: snapshotCard.displayTitle || getCardDisplayTitle(frontmatter.title, fileName),
    displayTitle: snapshotCard.displayTitle || getCardDisplayTitle(frontmatter.title, fileName),
    body,
    start: String(frontmatter.start || '').trim(),
    due: String(frontmatter.due || '').trim(),
    labels: Array.isArray(frontmatter.labels) ? frontmatter.labels.map((labelId) => String(labelId)) : [],
    frontmatter,
    timestamps,
    taskSummary: snapshotCard.taskSummary || getTaskListSummary(body),
    taskStartDates: snapshotCard.taskStartDates || getTaskListStartDates(body),
    incompleteTaskStartDates: snapshotCard.incompleteTaskStartDates || getIncompleteTaskListStartDates(body),
    taskDueDates: snapshotCard.taskDueDates || getTaskListDueDates(body),
    incompleteTaskDueDates: snapshotCard.incompleteTaskDueDates || getIncompleteTaskListDueDates(body),
    mtimeMs: getTimestampMs(timestamps.updatedAt),
    createdAt: timestamps.createdAt || '',
    updatedAt: timestamps.updatedAt || '',
  };
}
...
async function listCards(boardRoot, options = {}) {
  ...
  if (snapshot.errors.length > 0) {
    ...
    throw new Error(`Could not read board snapshot at ${firstError.path}: ${firstError.message}`);
  }

  const snapshotLists = snapshot.lists.map((list) => ({
    directoryName: list.listName,
    displayName: getListDisplayName(list.listName),
    path: list.listPath,
    isArchive: list.listName === ARCHIVE_DIRECTORY_NAME,
  ...
  const selectedLists = listRefs.length > 0
    ? listRefs.map((listRef) => resolveUniqueMatch(
      snapshotLists,
      listRef,
      [
        (item, ref, exact) => {
    ...
    : snapshotLists;
  const boardSettings = snapshot.boardSettings || await boardLabels.readBoardSettings(resolvedBoardRoot, { ensureFile: false });
  ...
  for (const listEntry of selectedLists) {
    const loaded = listEntry.cards.map((snapshotCard, index) => {
      cardOrder.set(`${listEntry.path}\n${snapshotCard.cardName}`, index);
      return cardRecordFromSnapshot(resolvedBoardRoot, listEntry, snapshotCard);
    });
...
async function resolveCard(boardRoot, options = {}) {
  ...
  if (path.isAbsolute(cardRef)) {
    const normalizedPath = path.resolve(cardRef);
    const pathMatches = cards.filter((card) => path.resolve(card.filePath) === normalizedPath);
    if (pathMatches.length === 1) {
      return pathMatches[0];
    }
    if (pathMatches.length > 1) {
      throw new Error(`Ambiguous card reference "${cardRef}".`);
    }
    throw new Error(`Could not find card: ${cardRef}`);
  }
...
async function createCard(boardRoot, options = {}) {
  ...
  const fileName = `${sanitizeCardSlug(title).slice(0, 25)}-${randomSuffix()}.md`;
...
function getCanonicalSignboardIdFromUri(value) {
  try {
    const parsed = new URL(String(value || '').trim());
    return parsed.protocol === 'signboard:' && parsed.hostname === 'open-card'
...
function normalizeMovedCardFrontmatter(boardRoot, cardPath, frontmatter) {
  const sourceId = String(frontmatter && frontmatter.signboard_id || '').trim()
    || getCanonicalSignboardIdFromUri(frontmatter && frontmatter.signboard_uri);
  const normalized = obsidianIntegration.normalizeSignboardCardFrontmatter({
    boardRoot,
    cardPath,
    frontmatter,
  });

  // Moving a card changes its list metadata, not its identity. The Obsidian
  // normalizer intentionally removes IDs from ad-hoc filenames, so preserve
  // an existing explicit ID when a legacy card already has one.
  if (sourceId) {
    normalized.signboard_id = sourceId;
    normalized.signboard_uri = `signboard://open-card?id=${encodeURIComponent(sourceId)}`;
  }
...
async function duplicateCard(boardRoot, options = {}) {
  ...
  // A duplicate is a new card. Preserve a Signboard identity only when the
  // source had one, but never carry its ID or canonical URI into the copy.
  const sourceSignboardId = String(sourceCard.frontmatter?.signboard_id || '').trim();
  const sourceHasCanonicalUri = String(sourceCard.frontmatter?.signboard_uri || '').trim().startsWith('signboard://open-card');
  if (sourceSignboardId || sourceHasCanonicalUri) {
    const nextCardId = getCardId(fileName);
    if (nextCardId) {
      frontmatterSource.signboard_id = nextCardId;
      frontmatterSource.signboard_uri = `signboard://open-card?id=${encodeURIComponent(nextCardId)}`;
    }
  }
    ...
    if (nextPath !== cardRecord.filePath) {
      if (
        ...
        await cardFrontmatter.writeCard(nextPath, {
          frontmatter: normalizeMovedCardFrontmatter(
            resolvedBoardRoot,
            nextPath,
            applyCardListMove(movedCard.frontmatter, {
