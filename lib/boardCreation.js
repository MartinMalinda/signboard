...
const { writeOrderManifest } = require('./orderManifest');
const { writeBoardSettings } = require('./boardLabels');
const { createDefaultNewBoardV2Profile } = require('../shared/v2BoardProfile');
...
const DEFAULT_WELCOME_CARD_FILE = 'hello-stock.md';
...
function buildStarterCardBody() {
  ...
  return [
    ...
    '- Open the header search to switch boards or search current-board cards.',
    ...
    `- [ ] (due: ${thirdDueDate}) Review these dated checklist items in your board views`,
    '- [x] Opened Signboard and kicked the tires',
    '',
    '## Things worth trying',
    '',
    '- Use the bottom view dock to switch between Kanban and Table.',
    ...
    '- Cmd/Ctrl + F opens the quick switcher and focuses its search field',
...
async function createBoard(boardRootInput, options = {}) {
  ...
  try {
    for (const listName of DEFAULT_BOARD_LIST_NAMES) {
      ...
      await writeOrderManifest(path.join(boardRoot, listName), []);
    }

    await writeOrderManifest(boardRoot, DEFAULT_BOARD_LIST_NAMES);
    await writeBoardSettings(boardRoot, {
      v2: createDefaultNewBoardV2Profile(),
    });
    ...
    if (seedWelcomeCard) {
      ...
      await writeOrderManifest(path.join(boardRoot, DEFAULT_BOARD_LIST_NAMES[0]), [cardFile]);
    }
  } catch (error) {
...
async function initializeNewBoard(boardRootInput) {
  const boardRootInputValue = String(boardRootInput || '').trim();
  const boardRoot = path.resolve(boardRootInputValue);
  if (!boardRootInputValue || !boardRoot || boardRoot === path.dirname(boardRoot)) {
    throw new Error('Board root must include a directory name.');
  }

  return writeBoardSettings(boardRoot, {
    v2: createDefaultNewBoardV2Profile(),
  });
}
