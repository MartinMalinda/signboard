...
function buildSignboardCardPathUri(cardPath) {
  const normalizedPath = trimString(cardPath).replace(/\\/g, '/').replace(/^\/+/, '');
  return normalizedPath ? `signboard://open-card?path=${encodeURIComponent(normalizedPath)}` : '';
}

function getRelativeCardPath(boardRoot, cardPath) {
  const normalizedBoardRoot = path.resolve(trimString(boardRoot));
  const normalizedCardPath = path.resolve(trimString(cardPath));
  if (!normalizedBoardRoot || !normalizedCardPath) {
    return '';
  }

  const relativePath = path.relative(normalizedBoardRoot, normalizedCardPath);
  if (
    !relativePath ||
    relativePath === '..' ||
...
function buildSignboardCardUriForCard({
  boardRoot,
  cardPath,
  frontmatter = {},
} = {}) {
  const fileId = getCardFileId(cardPath);
  if (fileId) {
    return buildSignboardCardUri(fileId);
  }

  const relativePath = getRelativeCardPath(boardRoot, cardPath);
  return buildSignboardCardPathUri(relativePath) || buildSignboardCardUri(frontmatter.signboard_id);
}
...
function normalizeSignboardCardFrontmatter({
  ...
  if (getCardFileId(cardPath)) {
    source.signboard_id = cardId;
    source.signboard_uri = buildSignboardCardUri(cardId);
  } else {
    delete source.signboard_id;
    source.signboard_uri = buildSignboardCardUriForCard({
      boardRoot,
      cardPath,
      frontmatter: source,
    });
...
async function writeObsidianBaseFile(boardRoot) {
  ...
  await atomicWriteFile(basePath, baseYaml, 'utf8');
  return {
    ok: true,
    basePath,
    vaultRoot,
...
function buildLinkedNoteContent({
  ...
  const linkedFrontmatter = {
    ...
    signboard_uri: signboardUri || buildSignboardCardUri(cardId),
  };
  if (cardId) {
    linkedFrontmatter.signboard_card_id = cardId;
  }

  const frontmatter = yaml.dump(linkedFrontmatter, {
    schema: yaml.JSON_SCHEMA,
    lineWidth: -1,
    noRefs: true,
    noCompatMode: true,
...
async function createLinkedObsidianNote({
  ...
  const cardId = getCardFileId(resolvedCardPath);
  const signboardUri = buildSignboardCardUriForCard({
  ...
  await atomicWriteFile(notePath, buildLinkedNoteContent({
    cardId,
    signboardUri,
    boardName,
  }), 'utf8');
...
async function createLinkedObsidianNoteAtPath({
  ...
  await atomicWriteFile(resolvedNotePath, buildLinkedNoteContent({
    cardId,
    signboardUri,
    boardName,
  }), 'utf8');
