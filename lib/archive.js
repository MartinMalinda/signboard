...
const { listOrderedEntries, removeFromOrderManifest, writeOrderManifest } = require('./orderManifest');
...
async function archiveCard(boardRoot, cardPath) {
  ...
  await removeFromOrderManifest(sourceListPath, [path.basename(normalizedCardPath)]);
...
async function archiveList(boardRoot, listPath) {
  ...
  await removeFromOrderManifest(normalizedBoardRoot, [originalDirectoryName]);
...
async function restoreArchivedList(boardRoot, archivedListPath, restoredDirectoryName = '') {
  ...
  if (!orderedLists.includes(nextDirectoryName)) {
    await writeOrderManifest(normalizedBoardRoot, [...orderedLists, nextDirectoryName]);
  }
