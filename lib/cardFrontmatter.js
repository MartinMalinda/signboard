...
const { deriveCardTitleFromFileName, getCardDisplayTitle } = require('./cardTitle');
...
const KNOWN_KEYS = new Set(ORDERED_KEYS);
...
function orderFrontmatter(frontmatter) {
  ...
  if (typeof frontmatter.start === 'string' && frontmatter.start.trim().length > 0) {
    ordered.start = frontmatter.start.trim();
  }
  ...
  for (const key of ORDERED_KEYS) {
    if (key === 'title' || key === 'start' || key === 'due' || key === 'labels') {
      continue;
    }
...
function normalizeFrontmatter(frontmatter = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(source, 'signboard_v2')) {
    const normalizedV2 = normalizeV2CardMetadata(source.signboard_v2);
    if (normalizedV2) source.signboard_v2 = normalizedV2;
    else delete source.signboard_v2;
  }
  ...
  if (source.start == null) {
    if (source.scheduled != null) {
      source.start = source.scheduled;
    } else if (source.startDate != null) {
      source.start = source.startDate;
    } else if (source.scheduledDate != null) {
      source.start = source.scheduledDate;
    } else if (source['Start-date'] != null) {
      source.start = source['Start-date'];
    } else if (source.Start != null) {
      source.start = source.Start;
    } else if (source.Scheduled != null) {
      source.start = source.Scheduled;
    }
  }
  ...
  delete source.scheduledDate;
  delete source['Start-date'];
  delete source.Start;
  delete source.Scheduled;
  ...
  const start = toIsoDateString(source.start);
  if (start === undefined) {
    delete source.start;
  } else {
    source.start = start;
...
function deriveTitleFromPath(filePath) {
  return deriveCardTitleFromFileName(filePath);
}
...
function frontmatterForWrite(frontmatter, filePath) {
  ...
  if (normalized.start) {
    ordered.start = normalized.start;
  }
...
async function readCard(filePath) {
  ...
  return {
    ...
    body: parsed.body,
    displayTitle: getCardDisplayTitle(normalized.title, filePath),
  };
}
