...
async function runBoardsListCommand(positionals, options, context, usage) {
  assertAllowedOptions(options, ['json', 'board'], usage);
...
async function runCardsCommand(action, positionals, options, context) {
  ...
  if (action === 'create') {
    assertAllowedOptions(
      options,
      ['list', 'title', 'from-card', 'from-list', 'body', 'body-file', 'start', 'due', 'label', 'remove-label', 'clear-labels', 'kind', 'work-type', 'priority-class', 'effort-points', 'dry-run', 'json', 'board'],
    ...
    const created = fromCard
      ...
      : await createCard(boardRoot, {
          ...
          workType: getOption(options, 'work-type'),
