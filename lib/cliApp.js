...
function renderHelpText(commandName = 'signboard') {
  return [
    ...
    '',
    `  ${commandName} boards list [--json]`,
    `  ${commandName} boards discover [root] [--max-depth <n>] [--json]`,
    ...
    '',
    'Board discovery:',
    '  `boards --list` is a short alias for `boards list`',
    '  `boards discover .` searches project subdirectories without changing the selected board',
    '  --max-depth <n>      Search depth (default 8, maximum 16)',
    '  --include-ignored    Include Git-ignored directories when searching',
    '  --include-nested     Continue below a discovered board root',
    '  --use                Select the single discovered board',
    '',
    'Card list options:',
    ...
    '  --limit <n>',
    '  --include-archive',
    '  --no-body             Omit Markdown bodies from card JSON output',
    '  --summary             Alias for --no-body',
    '  --json',
    '',
    'Card write options:',
    '  --body <text>',
    ...
    '  --due <YYYY-MM-DD|none>',
    '  --label <ref>         create/duplicate only',
    '  --set-label <ref>     edit only, replaces labels',
    '  --add-label <ref>     edit only',
    '  --remove-label <ref>  edit/create-from-card/duplicate only',
...
function cardForJson(card, options = {}) {
  const payload = {
    // Legacy filenames do not always carry the five-character suffix. Keep
    // id useful for automation while retaining the raw filename separately.
    id: card.cardId || card.fileName,
    ...
    taskStartDates: card.taskStartDates,
    incompleteTaskStartDates: card.incompleteTaskStartDates,
...
async function resolveBoardRoot(options, context) {
  ...
  const cardPathInput = getOption(options, 'card') || getOption(options, 'from-card');
  if (cardPathInput && path.isAbsolute(String(cardPathInput))) {
    const inferredBoardRoot = await inferBoardRootFromCardPath(String(cardPathInput));
    if (inferredBoardRoot) {
      return inferredBoardRoot;
    }

    throw new Error(`Could not infer a Signboard board from card path: ${cardPathInput}. Pass --board <path> explicitly.`);
  }
...
async function inferBoardRootFromCardPath(cardPathInput) {
  let candidatePath = path.dirname(path.resolve(cardPathInput));
...
function renderBoardsListText(discovery) {
  const boards = Array.isArray(discovery.boards) ? discovery.boards : [];
  if (boards.length === 0) {
    return [
      'No known boards found.',
      'Open a board in Signboard, run `signboard use /path/to/board`, or pass `--board <path>` to board commands.',
    ].join('\n');
  }

  return boards.map((board) => {
    const labels = [];
    if (board.isActive) labels.push('active');
    if (board.isCurrent) labels.push('current');
    if (board.isOpen) labels.push('open');
    if (board.isTrusted) labels.push('trusted');
    if (!board.exists) labels.push('missing');

    const marker = board.isActive || board.isCurrent ? '*' : '-';
    const labelText = labels.length > 0 ? ` (${labels.join(', ')})` : '';
    return `${marker} ${board.name}${labelText}\n  ${board.boardRoot}`;
  }).join('\n');
}

async function runBoardsListCommand(positionals, options, context, usage) {
  assertAllowedOptions(options, ['json', 'board', 'list'], usage);
  assertNoExtraPositionals(positionals, 0, usage);

  const explicitBoard = getOption(options, 'board');
  const currentBoardRoot = explicitBoard
    ? path.resolve(String(explicitBoard))
    : await getCurrentBoard(context.stateOptions);
  const trustedBoardRoots = await readDesktopTrustedBoardRoots();
  const openBoardsState = await readDesktopOpenBoardsState();
  const discovery = await discoverBoards({
    trustedBoardRoots,
    openBoardRoots: openBoardsState.openBoardRoots,
    activeBoardRoot: openBoardsState.activeBoardRoot,
    currentBoardRoot,
    includeAllowedRootScan: false,
    allowWhenNoAllowedRoots: true,
  });

  if (getOption(options, 'json') === true) {
    context.stdout.write(`${JSON.stringify(discovery, null, 2)}\n`);
    return 0;
  }

  context.stdout.write(`${renderBoardsListText(discovery)}\n`);
  return 0;
}

function renderBoardsDiscoveryText(discovery) {
  const boards = Array.isArray(discovery.boards) ? discovery.boards : [];
  const lines = boards.length > 0
    ? boards.flatMap((board) => [
      `- ${board.name} (${board.cardCount} cards, ${board.listCount} lists, ${board.confidence})`,
      `  ${board.path} -> ${board.absolutePath}`,
    ])
    : ['No Signboard projects found.'];
...
async function runBoardsDiscoverCommand(positionals, options, context, usage) {
  assertAllowedOptions(options, ['json', 'max-depth', 'include-ignored', 'include-nested', 'use'], usage);
  assertNoExtraPositionals(positionals, 1, usage);

  const discovery = await discoverBoardProjects(positionals[0] || '.', {
    maxDepth: getOption(options, 'max-depth'),
    includeIgnored: getOption(options, 'include-ignored') === true,
    includeNested: getOption(options, 'include-nested') === true,
  });

  if (getOption(options, 'use') === true) {
    let selectedBoard = discovery.boards.length === 1 ? discovery.boards[0] : null;
    const jsonOutput = getOption(options, 'json') === true;
    if (!selectedBoard && !jsonOutput && process.stdin.isTTY && process.stdout.isTTY && discovery.boards.length > 1) {
      context.stdout.write(renderBoardsDiscoveryText(discovery));
      context.stdout.write('\nSelect a board number to use (or press Enter to cancel):\n');
      discovery.boards.forEach((board, index) => {
        context.stdout.write(`  ${index + 1}. ${board.path}\n`);
      });
      ...
      if (Number.isInteger(selectedIndex) && discovery.boards[selectedIndex]) {
        selectedBoard = discovery.boards[selectedIndex];
      }
    }

    if (!selectedBoard) {
      throw new Error(`--use requires exactly one discovered board; found ${discovery.boards.length}.`);
    }

    await setCurrentBoard(selectedBoard.absolutePath, context.stateOptions);
    discovery.currentBoardUpdated = true;
  }
...
async function runBoardsCommand(action, positionals, options, context) {
  const usage = [
    `Usage: ${context.commandName || 'signboard'} boards list [--json]`,
    `       ${context.commandName || 'signboard'} boards discover [root] [--max-depth <n>] [--json]`,
    `       ${context.commandName || 'signboard'} boards create <board-root> [--no-welcome] [--use] [--json]`,
    `       ${context.commandName || 'signboard'} boards create --parent <parent-root> --name <board-name> [--no-welcome] [--use] [--json]`,
  ].join('\n');

  if ((!action && (options.list === undefined || getOption(options, 'list') === true)) || action === 'list') {
    return runBoardsListCommand(positionals, options, context, usage);
  }

  if (action === 'discover') {
    return runBoardsDiscoverCommand(positionals, options, context, usage);
  }
...
async function runCardsCommand(action, positionals, options, context) {
  ...
  if (!action || action === 'list' || !CARD_SUBCOMMANDS.has(action)) {
    assertAllowedOptions(
      options,
      ['list', 'label', 'label-mode', 'search', 'due', 'due-source', 'task-status', 'sort', 'limit', 'include-archive', 'no-body', 'summary', 'json', 'board'],
    ...
    if (getOption(options, 'json') === true) {
      const includeBody = getOption(options, 'no-body') !== true && getOption(options, 'summary') !== true;
      context.stdout.write(`${JSON.stringify(cards.map((card) => cardForJson(card, { includeBody })), null, 2)}\n`);
      return 0;
    }
  ...
  if (action === 'read') {
    assertAllowedOptions(options, ['card', 'list', 'include-archive', 'no-body', 'summary', 'json', 'board'], 'Usage: signboard cards read --card <card-ref> [--list <list-ref>]');
    ...
    const includeBody = getOption(options, 'no-body') !== true && getOption(options, 'summary') !== true;
    context.stdout.write(`${JSON.stringify(cardForJson(card, { includeBody }), null, 2)}\n`);
    return 0;
  }
  ...
  if (action === 'create') {
    assertAllowedOptions(
      options,
      ['list', 'title', 'from-card', 'from-list', 'body', 'body-file', 'start', 'due', 'label', 'remove-label', 'clear-labels', 'kind', 'priority-class', 'effort-points', 'dry-run', 'json', 'board'],
    ...
    if (!fromCard && !String(title || '').trim()) {
      throw new Error('--title is required and must be non-empty when creating a CLI card.');
    }
    const created = fromCard
      ? await duplicateCard(boardRoot, {
          ...
          ...(body !== undefined ? { body } : {}),
          ...(Object.prototype.hasOwnProperty.call(options, 'start') ? { start: getOption(options, 'start') } : {}),
      ...
      : await createCard(boardRoot, {
          ...
          title,
          body,
          start: getOption(options, 'start'),
          due: getOption(options, 'due'),
          labelRefs: getOptionList(options, 'label'),
          kind: getOption(options, 'kind'),
          priorityClass: getOption(options, 'priority-class'),
          effortPoints: getOption(options, 'effort-points'),
          dryRun: getOption(options, 'dry-run') === true,
        });
  ...
  if (action === 'duplicate') {
    assertAllowedOptions(
      options,
      ['card', 'from-list', 'list', 'title', 'body', 'body-file', 'start', 'due', 'label', 'remove-label', 'clear-labels', 'dry-run', 'json', 'board'],
  ...
  if (action === 'edit') {
    assertAllowedOptions(
      options,
      ['card', 'list', 'title', 'body', 'body-file', 'append-body', 'replace-section', 'insert-after-heading', 'text', 'text-file', 'start', 'due', 'set-label', 'add-label', 'remove-label', 'clear-labels', 'move-to', 'dry-run', 'json', 'board'],
    ...
    if (Object.prototype.hasOwnProperty.call(options, 'start')) {
      editOptions.start = getOption(options, 'start');
    }
...
async function runCliCommand(argv, context = {}) {
...
function cliJsonRequested(argv) {
  return argv.some((arg) => arg === '--json' || String(arg).startsWith('--json='));
}

function writeCliJsonError(error, stderr) {
  const payload = {
    error: {
      code: String(error && error.code || 'CLI_ERROR'),
      message: error && error.message ? String(error.message) : String(error || 'Unknown CLI error'),
    },
  };
...
async function runCli(argv, context = {}) {
  try {
    return await runCliCommand(argv, context);
  } catch (error) {
    if (cliJsonRequested(argv)) {
      writeCliJsonError(error, context.stderr || process.stderr);
      return 1;
    }
