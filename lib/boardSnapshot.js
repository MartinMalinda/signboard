...
function getV2StageForList(profile, listName) {
  const stages = isObject(profile && profile.stages) ? profile.stages : {};
  const matches = V2_STAGE_KEYS.filter((stage) => (
    Array.isArray(stages[stage]) && stages[stage].includes(listName)
  ));
...
function buildV2CardProjection(frontmatter, cardName, listName, profile) {
  ...
  const status = getV2StageForList(profile, listName);
  ...
  const hasWorkType = Object.prototype.hasOwnProperty.call(source, 'work_type');
  const normalizedWorkType = hasWorkType
    ? normalizeV2MetadataValue(source, 'work_type', (value) => typeof value === 'string' && value.trim() ? value.trim().toLowerCase() : null)
    : (cardDefaults.workType || null);
  if (hasWorkType && normalizedWorkType === null) metadataWarnings.push('INVALID_WORK_TYPE');
  const metadata = {
    ...source,
