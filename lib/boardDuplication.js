...
const { readOrderManifest, writeOrderManifest } = require('./orderManifest');

const CARD_ID_ALPHABET = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789';
const CARD_FILE_NAME_PATTERN = /^(\d{3}-.+-)([A-Za-z0-9]{5})(\.md)$/;
...
function isCardMarkdownPath(boardRoot, filePath) {
  ...
  if (fileName === BOARD_SETTINGS_FILE_NAME) {
    return false;
  }
...
async function rewriteCopiedOrderManifests(cards) {
  const cardsByDirectory = new Map();
  for (const card of cards) {
    const directoryPath = path.dirname(card.newPath);
    const mappings = cardsByDirectory.get(directoryPath) || new Map();
    mappings.set(path.basename(card.oldPath), path.basename(card.newPath));
    cardsByDirectory.set(directoryPath, mappings);
  }

  for (const [directoryPath, mappings] of cardsByDirectory) {
    ...
    await writeOrderManifest(
      directoryPath,
      manifest.order.map((entryName) => mappings.get(entryName) || entryName),
    );
  }
}
...
async function duplicateBoard(options = {}) {
  ...
  await rewriteCopiedOrderManifests(cards);
