...
onMounted(() => { document.addEventListener('keydown', onKeydown); document.addEventListener('pointerdown', onPointerdown); if (props.isOpen) { wasOpen = true; restoreTarget = props.opener || (document.activeElement instanceof HTMLElement ? document.activeElement : null); focusFirst() } })
onUpdated(() => {
  if (props.isOpen && !wasOpen) { wasOpen = true; restoreTarget = props.opener || (document.activeElement instanceof HTMLElement ? document.activeElement : null); focusFirst() }
  else if (!props.isOpen && wasOpen) { wasOpen = false; void nextTick(() => (props.opener || restoreTarget)?.focus()); restoreTarget = null }
})
...
<template>
  <Teleport to="body">
    <div ref="root" :id="props.id || undefined" :hidden="!isOpen" :class="['label-popover', 'app-popover', props.className, { hidden: !isOpen }]" data-sb-modal-layer role="menu" :aria-hidden="!isOpen" :aria-label="ariaLabel || undefined" style="position: fixed" @click.stop>
      <slot />
    </div>
  </Teleport>
</template>
