...
const boardPath = ref(''); const listPath = ref(''); const lists = ref<string[]>([]); const name = ref(''); const selectedLabels = ref<string[]>([]); const saving = ref(false); const v2Profile = ref<V2BoardProfile>({}); const workOpen = ref(false); const kind = ref('task'); const priorityClass = ref('P2'); const effortPoints = ref(''); let wasOpen = false
...
function applyV2Defaults() { kind.value = String(v2Defaults.value.kind || 'task'); priorityClass.value = String(v2Defaults.value.priorityClass || 'P2'); effortPoints.value = '' }
...
function creationFrontmatter() { const frontmatter: Record<string, unknown> = selectedLabels.value.length ? { labels: [...selectedLabels.value] } : {}; if (v2Enabled.value) { const metadata: Record<string, unknown> = { contract_version: 1, kind: kind.value, priority_class: priorityClass.value }; if (effortPoints.value) metadata.estimate = { effort_points: Number(effortPoints.value) }; frontmatter.signboard_v2 = metadata }; return Object.keys(frontmatter).length ? frontmatter : undefined }
...
<template>
  <Modal id="modalAddCardToList" modal-class="quick-add-card-modal" :is-open="isOpen" :on-close="onClose" positioning="fixed" :overflow="true" :show-chrome="false" labelled-by="quickAddHeading" initial-focus="#userInputCardName">
    <form class="modal-content quick-add-card-form" @submit.prevent="submit()">
      ...
      <div class="quick-add-field">
        ...
        <p class="modal-hint">This seeds the filename. The title is optional and can be set after creation.</p>
      </div>
