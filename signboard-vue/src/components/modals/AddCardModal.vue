...
const ui = useUiStore(); const data = useBoardDataStore(); const name = ref(''); const selectedLabels = ref<string[]>([]); const saving = ref(false); const workOpen = ref(false); const kind = ref('task'); const workType = ref('product'); const priorityClass = ref('P2'); const effortPoints = ref('')
...
function resetWorkDetails() { kind.value = String(v2Defaults.value.kind || 'task'); workType.value = String(v2Defaults.value.workType || 'product'); priorityClass.value = String(v2Defaults.value.priorityClass || 'P2'); effortPoints.value = ''; workOpen.value = false }
