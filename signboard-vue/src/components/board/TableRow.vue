...
<template>
  <tr class="board-table-row" :class="{ 'is-selected': props.selected }" :data-path="props.entry.cardPath" :data-list-path="props.entry.listPath" :aria-selected="props.selected ? 'true' : 'false'" @click="open">
    ...
    <td v-for="column in ['priority_index', 'risk_reduction_index', 'impact_index', 'autonomy_score', 'quick_win_index', 'human_leverage_index']" :key="column" class="board-table-cell" :class="`board-table-cell-${column}`"><span :class="{ 'board-table-empty-value': props.entry[column] === null }">{{ scoreText(props.entry[column]) }}</span></td>
  </tr>
</template>
