...
- Small counts: Critical, Ready, Blocked, Agent-ready.
...
2. Next best work: the top few eligible P2 cards, regardless of autonomy.
3. Low-hanging fruit: quick wins with visible confidence and verification.
4. Agent loops: cards safe for an autonomous implementation loop.
5. Blocked: compact warning section when non-empty.
...
- Empty sections explain their eligibility rule rather than looking broken.

# Accepted decision

Use five stable sections in this order: `critical`, `next_best_work`, `low_hanging_fruit`, `agent_loops`, and `blocked`. Show at most three cards per section, with title, stage, one compact signal, and a one-line reason. Keep Critical and Blocked visually distinct, expose Why this appears on demand, and provide an Unshaped affordance for legacy or unscored cards. Results must come from the shared snapshot projection; profile-disabled boards remain unchanged.
