...
List and card results follow the order stored in per-directory `.board.json` manifests; the board root manifest also stores board settings under `settings`. Moving or reordering a card preserves its filename.
...
- `signboard_list_board_views` reports the board-scoped Kanban and Table views.
