...
`Create Signboard` asks for confirmation before it changes anything. It adds a root `.board.json` manifest with default board settings, creates Signboard list folders when needed, treats existing direct child folders as lists, moves top-level Markdown notes into `To-do`, adds Signboard frontmatter to cards, and opens the folder with `signboard://open-board?path=...`.
